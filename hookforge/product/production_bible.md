...
## 4.14 작업 승인 카드 프로토콜 (2026-07-28 캡틴 지시)
- 크레딧 소모 신규 생성·새 유형·대형 작업은 **작업 전** 카드 제시: ①클로드 모델 권장(페이블5/낮은 모델 — 캡틴이 세션 설정, 에이전트 자가 변경 불가) ②생성 모델·해상도·수량·총 크레딧 ③진행 조건(승인 대기/기승인 자동)
- 기승인 자동 범위: 2시간 순찰, 로그/백업 동기화, 예약된 발송 배치

## 4.15 고위험 모션 금지 + 육안 게이트 (2026-07-28 이어폰 소실 사고)
**사고**: 테크 데모(이어폰)에서 이어폰을 귀로 가져가는 순간 이어폰 헤드가 소실됨. 캡틴이 잡아냄. 프롬프트 설계 단계에서 걸렀어야 함.
**근본원인**: 작은 물체가 손·신체에 가려지며 이동하는 순간 = AI 비디오가 물체 형태·존재를 소실/왜곡시키는 최고위험 샷.
- **금지 모션 목록 (화면에 절대 담지 않음)**:
  - 소형 물체를 신체에 삽입·장착하는 동작: 이어폰→귀, 렌즈→눈, 반지→손가락, 이어링→귀, 알약→입안 근접
  - 소형 물체가 손가락·손바닥·머리카락에 가려지며 이동하는 구간
  - 물체가 프레임 밖으로 나갔다 다시 들어오는 구간(재등장 시 형태 붕괴)
- **대체 설계 원칙**:
  - 장착 결과만 보여줌 (이미 착용된 상태로 시작), 삽입 과정은 컷
  - 소형 제품은 케이스/트레이/손바닥 위 **정지 상태**로 히어로, 카메라가 움직여 디테일 표현
  - 반응(리액션)·사운드로 감정 전달, 물체 조작 최소화
- **육안 게이트 (절대규칙)**: 이 환경에선 에이전트가 렌더 프레임을 안정적으로 되받아 화질검수 불가(원격 샌드박스+전송 차단/절단). 따라서 **어떤 공개 게시물도 캡틴의 실제 렌더 영상 육안 승인 없이 발송·게시 금지**. 에이전트 QC = 설계단계 예방(본 규칙)만 담당, 최종 화질판정 = 캡틴.
- **크레딧 보호**: 생성 전 프롬프트·샷리스트를 카드와 함께 먼저 제시 → 캡틴이 설계 승인 후 생성 → 렌더 후 육안 승인 → 게시. 설계 미승인 상태 생성 금지(코인 낭비 방지).
